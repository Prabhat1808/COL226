...
typeElaborates([],parallel(letdef(X,_),letdef(X,_)),T) :- fail.

%----------------------------------------------
%elaborates : for different types of definitions
elaborates(Gamma,Gamma1,letdef(X,E),[(X,T)|Gamma]) :- hastype(Gamma1,E,T).
...
hastype(Gamma,eq(X,Y),boolT) :- hastype(Gamma,X,intT) , hastype(Gamma,Y,intT).
