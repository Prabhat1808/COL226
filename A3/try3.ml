...
let rec compose_in_situ sub1 sub2 =
	Hashtbl.iter (fun key value -> if((Hashtbl.find_all sub1 key) = []) then (Hashtbl.add sub1 key value) ) sub2;;
...
let rec mgu t1 t2 = 
	...
	match (t1,t2) with
	(V x,V y) -> ((Hashtbl.add mgu_sub x (V y)) ; mgu_sub)

	| (V x, Node (sym, tl)) -> (
							if((second_el sym) = 0)
								then ((Hashtbl.add mgu_sub x (Node (sym,tl))) ; mgu_sub)
							else
								(if (mem x (vars (Node(sym,tl)))) then raise NOT_UNIFIABLE
										else (((Hashtbl.add mgu_sub x (Node (sym,tl)))) ; mgu_sub)
								))
	| (Node (sym,tl) , V x) -> mgu (V x) (Node (sym,tl))

	| (Node (sym1,tl1), Node(sym2, tl2)) -> (
											if ((second_el sym1) = 0) then
												(
													if (not ((first_el sym1) = (first_el sym2))) then
														raise NOT_UNIFIABLE
												)
											else 
												begin
													if (not ((first_el sym1) = (first_el sym2))) then 
														...
														| (x::xs , y::ys) -> (unify (compose_in_situ sign (mgu x y)) (map (mgu x y) xs) (map (mgu x y) ys))
														| ([],[]]) -> compose_in_situ (Hashtbl.create 1) sign
														in

														unify mgu_sub tl1 tl2
