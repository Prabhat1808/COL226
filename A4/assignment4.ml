...
exception InvalidOperandEv1 ;;
exception InvalidOperandEv2 ;;
exception InvalidOperandKr ;;
...
let rec close_eval2 op in1 in2 = match (op,in1,in2) with
		...
		| ("Eq", CLOSE (C b1 , envr) , CLOSE (C b2 , envr1)) -> CLOSE (BL (b1 = b2) , envr)
		| ("Gt", CLOSE (C b1 , envr) , CLOSE (C b2 , envr1)) -> CLOSE (BL (b1 > b2) , envr)
		| ("Ge", CLOSE (C b1 , envr) , CLOSE (C b2 , envr1)) -> CLOSE (BL (b1 >= b2) , envr)
		| ("Le", CLOSE (C b1 , envr) , CLOSE (C b2 , envr1)) -> CLOSE (BL (b1 <= b2) , envr)
		| ("Lt", CLOSE (C b1 , envr) , CLOSE (C b2 , envr1)) -> CLOSE (BL (b1 < b2) , envr)
		| _ -> raise InvalidOperandEv2
	;;

let rec close_eval3 fn elst envr stk = match elst with
	[] -> []
	| x :: xs -> ( match (fn (CLOSE (x,envr)) stk) with
				CLOSE (e,env) -> e :: (close_eval3 fn xs envr stk)  )
...
let rec krivine inp stk = match (inp,stk) with
		...
		| (CLOSE (Eq (e1,e2) , envr),s) -> close_eval2 "Eq" (krivine (CLOSE (e1, envr))  s) (krivine (CLOSE (e2, envr))  s)
		...
		| (CLOSE (Tuple e1,envr),s) -> CLOSE(Tuple (close_eval3 krivine e1 envr s), envr)
		| (CLOSE (Proj (n,Tuple e1),envr),s) -> krivine (CLOSE (List.nth e1 n,envr)) s
...
secd ([],tabl,compile g,[]);;
krivine (CLOSE (g,tabl_kr)) [];;

secd ([],tabl,compile h,[]);;
krivine (CLOSE (h,tabl_kr)) [];;
...
let l1 = Lambda ("x",Sum(Prod (Id "x", C 5),C 3));;
let l2 = Call (l1,C 2);;

secd ([],tabl,compile l2,[]);;
krivine (CLOSE (l2,tabl_kr)) [];;

let l3 = If ((Eq (Id "x", C 5)),e,f);;

secd ([],tabl,compile l3,[]);;
krivine (CLOSE (l3,tabl_kr)) [];;

let l4 = Let ("x",C 3);;

secd ([],tabl,compile l4,[]);;
krivine (CLOSE (l4,tabl_kr)) [];;

let l5 = If ((Eq (Id "x", C 3)),e,f);;

secd ([],tabl,compile l5,[]);;
krivine (CLOSE (l5,tabl_kr)) [];;

let l6 = LetLoc ("x", C 3, Sum (Id "x",C 5));;

secd ([],tabl,compile l6,[]);;
krivine (CLOSE (l6,tabl_kr)) [];;
